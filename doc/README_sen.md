...
- **Сообщение от мастера**:
  ...
  ```
#### 4. Передача и получение данных дисплеем

Дисплей отправляет сообщение в SYNC окно и получает ответ от мастера с информацией о количестве свободных мест.

- **Команда дисплея**: 0x83
- **Адрес дисплея**: 0x40 (fixed)
- **Сообщение от дисплея**:

    | 0x02 | 0x40 | 0x83 | Данные | CS | 0x03 |

Процесс:

1. Дисплей отправляет запрос в своём тайм-слоте.
2. Мастер возвращает ответ с числом свободных мест.
